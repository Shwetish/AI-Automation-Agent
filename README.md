...
* **Autonomous Tool Usage:** Uses the **ReAct (Reasoning + Acting)** pattern to decide when to search the web vs. when to use local tools.
* **Live Web Research:** Fetches up-to-date web data using zero-cost DuckDuckGo search integration.
* **Custom Python Tools:** Extensible architecture using LangChain's `@tool` decorator (e.g., custom word counter and calculator).
* **Automated Workflow Logging:** Captures step-by-step reasoning, executed tool calls, and final outputs in `agent_workflow.log`.
